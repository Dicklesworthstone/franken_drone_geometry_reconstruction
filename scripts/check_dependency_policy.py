...
for required, expected in (("exclusive_async_runtime", "asupersync"), ("fdgr_unsafe_code", "forbid"), ("c_cpp_ffi", False), ("in_process_python", False), ("hosted_github_actions_authority", False)):
    if policy.get(required) != expected:
        fail(f"dependency policy {required} must equal {expected!r}")
forbidden = {row["name"] for row in policy.get("forbidden", []) if isinstance(row, dict) and isinstance(row.get("name"), str)}
fundamental = {row["name"] for row in policy.get("fundamental_exception", []) if isinstance(row, dict) and isinstance(row.get("name"), str)}
    ...
    for table in dependency_tables(document):
        for name, spec in table.items():
            if name in forbidden:
                fail(f"{manifest.relative_to(ROOT)}: forbidden dependency {name}")
            ...
            if isinstance(spec, dict):
                ...
                if version is not None and not (isinstance(version, str) and version.startswith("=") and len(version) > 1):
                    fail(f"{manifest.relative_to(ROOT)}: dependency {name} must pin an exact =version")
                if "git" in spec and (not isinstance(spec.get("rev"), str) or len(spec["rev"]) != 40):
...
if errors:
    ...
    raise SystemExit(1)
print(f"PASS: dependency policy ({len(forbidden)} forbidden names, {len(closure.get('planned_owned_sources', []))} exact research source identities)")
